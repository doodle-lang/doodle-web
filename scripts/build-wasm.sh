...
#
# The `wasm-opt -Oz` pass is not cosmetic: it is what the shipped artifact must carry
# so that the byte-exact size gate (scripts/wasm-ship-size.sh) measures what actually
# ships. Without it the demo and the @doodle-lang/engine package would ship the raw,
# larger wasm-bindgen output while the size discipline measured a smaller binary the
# pipeline never produced — so the optimize step is required here, not optional.
#
...
# target), wasm-bindgen, and wasm-opt (binaryen) on PATH, exactly as doodle-rust's own
# size gate does.
...
# Optimize the shipped `_bg.wasm` in place (same -Oz the size gate measures). Write to a
